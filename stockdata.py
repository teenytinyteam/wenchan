...
class StockData:
    ...
    def _load_source(self):
        ...
        if source_df is not None:
            self.data['source'] = source_df[['Date', 'Open', 'Close', 'Low', 'High',
    ...
    def _format_pivot_data(pivot_df):
        ...
        new_pivot_df = pd.DataFrame(columns=['Start', 'End', 'Low', 'High'])
        for index in range(0, len(pivot_df), 2):
            ...
            new_pivot_df.loc[start.name] = [start['Date'], end['Date'], high, low]
        return new_pivot_df
